...
static char kstring[] = "Some kernel memory";
...
int load_prog(int fd)
{
	...
	struct bpf_insn PROG[] = {
		...
		/* if r0 == 0 goto exit */
		BPF_JMP_IMM(BPF_JEQ, BPF_REG_0, 0, 4),          // Skip 4 instructions (goto exit) if not found
		BPF_MOV64_IMM(BPF_REG_1, BPF_REG_0),            /* 0: r1 = r0 (= map[0]) */
		BPF_MOV64_IMM(BPF_REG_2, sizeof(kstring)),      /* 1: r2 = sizeof(kstring) */
		BPF_MOV64_IMM(BPF_REG_3, &kstring),             /* 2: r3 = addressof(kstring) */
		BPF_EMIT_CALL(BPF_FUNC_probe_read),             /* 3: Try to read some memory definitely outside the sandbox */
...
void run(void)
{
	...
	if (*(uint64_t*)val == 0) {
		tst_res(TFAIL, "val unfilled!");
	} else {
		tst_res(TPASS, "val = %.4s", val);
	}
